...
#st_write(nests_county_cicada, "data/nests_county_cicada.shp", delete_layer = TRUE)
 
nests_county_cicada %>%
  st_drop_geometry() %>%
  drop_na(emergence_two) %>%
  write.csv("data/nestboxes_county_cicada.csv", row.names = FALSE)
...
nestboxes_county_cicada <- read.csv("data/nestboxes_county_cicada.csv", sep = ",",header = TRUE, stringsAsFactors = FALSE, row.names = NULL)
